...
game:GetService("Players").LocalPlayer:GetPropertyChangedSignal("Parent"):Connect(function()
    if not game:GetService("Players"):FindFirstChild(LocalPlayer.Name) then
        ...
        if PlayerCountLabel then PlayerCountLabel:Destroy() end
        
        -- Clean up ESP
        RemoveESPButton.Callback()
    end
end)
...
Rayfield:Notify({
    ...
    Content = "130+ Features Activated! Enjoy hacking!",
    Duration = 8,
    Image = 4483362458
})
...
task.spawn(function()
    ...
    ZoomSlider.Callback(ZoomSlider.CurrentValue)
    FOVSlider.Callback(FOVSlider.CurrentValue)
    TimeSlider.Callback(TimeSlider.CurrentValue)
end)
